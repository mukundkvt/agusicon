...
$regId  = trim((string)($input['reg_id'] ?? ''));
...
function buildPdfStream(string $name, string $email, string $amount, string $regId, string $approvedAt): string {
    $s   = '';
    $ML  = 50.0;
    $MR  = 545.28;
    $CW  = 495.28;
    ...
    // ── Reg ID box ───────────────────────────────────────────
    $displayRegId = $regId !== '' ? $regId : 'Manual Entry';
    $s .= fillR($ML, 715, $CW, 28, 0.937, 0.945, 0.980);
    $s .= bordR($ML, 715, $CW, 28, 0.765, 0.792, 0.914);
    $s .= pt(170, 724, 'Registration ID: ' . $displayRegId, 'F2', 12, $blue);

    // ── Event ────────────────────────────────────────────────
    $s .= pt(118, 697, 'AGUSICON 2026 - Bhadohi, Uttar Pradesh', 'F2', 12, $black);
    $s .= pt(152, 681, '25-26 July 2026   |   Blessing Garden, Bhadohi, UP', 'F1', 10, $muted);
    $s .= hln($ML, $MR, 667);
    ...
    $pData = [];
    $pData[] = ['Registration Category', 'All Other Delegates'];
    if ($amount !== '') {
        $pData[] = ['Amount Paid', $amount];
    }
    $pData[] = ['Confirmed On', date('d M Y, g:i A', strtotime($approvedAt))];
...
function buildPdf(string $name, string $email, string $amount, string $regId, string $approvedAt): string {
    $stream    = buildPdfStream($name, $email, $amount, $regId, $approvedAt);
...
// ── Send email ───────────────────────────────────────────────
function sendManualConfirmEmail(
    string $name, string $email, string $amount, string $regId,
    string $pdfBytes, string $approvedAt
): void {
    loadMailer();
    ...
    $mail->addBCC('agusicon2025@gmail.com', 'AGUSICON 2026');

    $displayRegId = $regId !== '' ? $regId : 'Manual Entry';
    $pdfName      = $regId !== '' ? "AGUSICON-Confirmation-{$regId}.pdf" : 'AGUSICON-Confirmation.pdf';
    $mail->addStringAttachment($pdfBytes, $pdfName, 'base64', 'application/pdf');

    $mail->CharSet = 'UTF-8';
    $mail->isHTML(true);
    $subjectRegId = $regId !== '' ? " - {$regId}" : '';
    $mail->Subject = "Payment Confirmed{$subjectRegId} | AGUSICON 2026, Bhadohi";

    $safeName        = htmlspecialchars($name,        ENT_QUOTES, 'UTF-8');
    $safeRegId       = htmlspecialchars($displayRegId, ENT_QUOTES, 'UTF-8');
    $safeApprovedAt  = htmlspecialchars($approvedAt,  ENT_QUOTES, 'UTF-8');

    $amountRow = $amount !== ''
        ? '<tr><td style="color:#6b7280;padding:5px 0;width:42%">Amount Paid</td>'
          . '<td style="font-weight:600;color:#1e293b">' . htmlspecialchars($amount, ENT_QUOTES, 'UTF-8') . '</td></tr>'
        : '';
...
<div style="max-width:620px;margin:0 auto;background:#fff;border-radius:14px;overflow:hidden;box-shadow:0 2px 16px rgba(0,0,0,0.09)">
  ...
  <div style="background:#e0f7fa;border-bottom:1px solid #80deea;padding:13px 36px;display:flex;justify-content:space-between;align-items:center">
    ...
    <span style="font-size:15px;font-weight:800;color:#115B86">{$safeRegId}</span>
  </div>

  <div style="padding:28px 36px">
    <p style="color:#1e293b;font-size:15px;margin:0 0 18px">Dear <strong>{$safeName}</strong>,</p>
    ...
    <div style="background:#f0fdf4;border:1.5px solid #86efac;border-radius:10px;padding:20px 24px;margin:0 0 20px">
      ...
      <table style="width:100%;border-collapse:collapse;font-size:13.5px">
        {$amountRow}
        <tr><td style="color:#6b7280;padding:5px 0;width:42%">Confirmed On</td><td style="font-weight:600;color:#1e293b">{$safeApprovedAt}</td></tr>
      </table>
    </div>
    ...
    <div style="background:#f8fafc;border:1px solid #e2e8f0;border-radius:10px;padding:18px 24px;margin:0 0 22px">
      ...
      <table style="width:100%;border-collapse:collapse;font-size:13.5px">
        ...
        <tr><td style="color:#6b7280;padding:4px 0">Dates</td><td style="color:#1e293b">25–26 July 2026</td></tr>
        <tr><td style="color:#6b7280;padding:4px 0">Venue</td><td style="color:#1e293b">Blessing Garden, Bhadohi, Uttar Pradesh</td></tr>
      </table>
    </div>
...
HTML;
    ...
    $altAmount   = $amount !== '' ? "Amount Paid    : $amount\n" : '';
    $mail->AltBody = "Dear $name,\n\nYour payment for AGUSICON 2026, Bhadohi is confirmed.\n\nRegistration ID : $displayRegId\n{$altAmount}Confirmed On   : $approvedAt\n\nPlease find the PDF confirmation letter attached.\n\nAGUSICON 2026 Secretariat\nagusicon2025@gmail.com";

    $mail->send();
}

// ── Log to CSV ──────────────────────────────────────────────
function appendManualLog(string $sentAt, string $name, string $email, string $amount, string $regId, string $status, string $error): void {
    $logPath = dirname(__DIR__) . '/data/manual-confirmations-log.csv';
    $dir     = dirname($logPath);
    if (!is_dir($dir)) {
        mkdir($dir, 0755, true);
    }
    $isNew   = !file_exists($logPath) || filesize($logPath) === 0;
    $fh      = fopen($logPath, 'ab+');
    if (!$fh) return;
    flock($fh, LOCK_EX);
    if ($isNew) fputcsv($fh, ['Sent At', 'Name', 'Email', 'Amount', 'Registration ID', 'Status', 'Error']);
    fputcsv($fh, [$sentAt, $name, $email, $amount, $regId, $status, $error]);
    fflush($fh);
    flock($fh, LOCK_UN);
    fclose($fh);
}
...
try {
    $pdfBytes = buildPdf($name, $email, $amount, $regId, $approvedAt);
    sendManualConfirmEmail($name, $email, $amount, $regId, $pdfBytes, $approvedAt);
    appendManualLog($approvedAt, $name, $email, $amount, $regId, 'Sent', '');
    echo json_encode(['ok' => true]);
} catch (Throwable $e) {
    appendManualLog($approvedAt, $name, $email, $amount, $regId, 'Failed', $e->getMessage());
